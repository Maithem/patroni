...
class ConfigHandler(object):
    ...
    def write_recovery_conf(self, recovery_params: CaseInsensitiveDict) -> None:
        ...
        if self._postgresql.major_version >= 120000:
            ...
            self._current_recovery_params.update(
                {p: ['', restart_required(p), self._postgresql_conf]
                 for p in self._recovery_parameters_to_compare
                 if p not in self._current_recovery_params})
        else:
